      ...
      <div class="row">
        <div class="col-md-12">
          ...
          <div class="card card-primary card-outline mb-4">
            ...
            <div class="card-body">
              {{-- Keep controls on one line at lg+ --}}
              <form id="marksFilterForm" method="GET" action="{{ route('teacher.marks-register.list') }}" class="row row-cols-lg-auto g-3 align-items-end">
                ...
                <div class="col">
                  <label class="form-label mb-1">Class (Assigned)</label>
...
<script>
(function(){
  // Optional grades (if controller passes them)
  ...
  // ----- Download handler (same as admin) -----
  ...
  if(btn && form){
    btn.addEventListener('click', function(){
      ...
      // Using the admin PDF endpoint which already exists
      const url = "{{ route('teacher.marks-register.download') }}" + "?" + params.toString();
      window.open(url, '_blank');
    });
  }

  // ----- Student dropdown (AJAX) same as admin) -----
  const classSel   = document.getElementById('class_id');
  const studentSel = document.getElementById('student_id');
  if (classSel && studentSel){
    const STUDENTS_URL_BASE = @json(route('teacher.marks-register.students', ['class' => 0]));
    ...
    // On load: hydrate students if class preselected
    ...
    // On change: reload
    classSel.addEventListener('change', () => loadStudentsForClass(classSel.value, ''));
  }
})();
</script>
@endpush
