...
public function download(Request $request)
{
    ...
    // Build hourly slots (07:00–19:00); tweak for your school day
    $slots = [];
    for ($h = 7; $h < 19; $h++) {
    ...
    // Optional: if you load remote images/fonts in the template
...
public function teacherDownload(Request $request)
{
    ...
    $request->validate([
        'class_id'   => ['required','integer'],
        'subject_id' => ['nullable','integer'], // optional: keep for future
    ]);
    ...
    // Slots (07:00–19:00). Adjust for your school day if needed.
    $slots = [];
    for ($h = 7; $h < 19; $h++) {
    ...
    // Build grid: [slotKey][week_id] = "Subject\nHH:MM - HH:MM\nRoom: 101"
    $grid = [];
    foreach ($slots as $k => $label) $grid[$k] = array_fill_keys($weekIds, '');
    foreach ($rows as $r) {
        if (!$r->start_time || !$r->end_time) continue;
